...
# 3. validate --- LA
def var_all_case_LA(
    model, base_dir, num_classes, patch_size=(112, 112, 80), stride_xy=18, stride_z=4
):
    ...
    image_list = [
        os.path.join(base_dir, "data", item, "mri_norm2.h5") for item in image_list
    ]
...
def var_all_case_Pancrease(
    ...
    image_list = [item.replace("\n", "") for item in image_list]
    image_list = [
        os.path.join(base_dir, "data", f"{item}_norm.h5") for item in image_list
    ]
