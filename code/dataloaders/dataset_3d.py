...
class WeakStrongAugment(object):
    def __init__(self, output_size, p_color=0.8, p_blur=0.2, flag_rot=True):
        ...
        self.randcolor = RandomBrightnessContrast(
            brightness_limit=0.5, contrast_limit=0.5, prob=p_color
        )
        self.randblur = RandomGaussianNoise(sigma=[0.1, 1.0], apply_prob=p_blur)
...
class RandomBrightnessContrast(object):
    def __init__(self, brightness_limit=0.5, contrast_limit=0.5, prob=0.8):
    ...
    def _random_update(self):
        self.alpha = (
            1.0 + np.random.uniform(-1.0 * self.contrast_limit, self.contrast_limit),
        )
        self.beta = 0.0 + np.random.uniform(
            -1.0 * self.brightness_limit, self.brightness_limit
        )
    ...
    def __call__(self, image):
        ...
        if np.random.uniform() < self.prob:
            ...
            image_norm = brightness_contrast_adjust(
                image_norm, alpha=self.alpha, beta=self.beta
            )
...
#     """
#     Apply random cutout to the image in a sample
#     Args:
#         cutout_size (tuple): Size of the cutout (width, height, depth)
#         n_holes (int): Number of holes to cut out
#         fill_value (int): Value to fill the cutout regions, default is 0
#     """

#     def __init__(self, cutout_size, n_holes=1, fill_value=0, with_sdf=False):
#         self.cutout_size = cutout_size  # (w, h, d)
...
#         # Apply n_holes cutouts
#         for _ in range(self.n_holes):
#             # Ensure the cutout can fit in the image
#             cutout_w = min(self.cutout_size[0], w)
#             cutout_h = min(self.cutout_size[1], h)
#             cutout_d = min(self.cutout_size[2], d)

#             # Randomly select the top-left corner of the cutout
#             w1 = np.random.randint(0, w - cutout_w + 1) if w > cutout_w else 0
#             h1 = np.random.randint(0, h - cutout_h + 1) if h > cutout_h else 0
#             d1 = np.random.randint(0, d - cutout_d + 1) if d > cutout_d else 0

#             # Create the cutout
#             image_copy[w1 : w1 + cutout_w, h1 : h1 + cutout_h, d1 : d1 + cutout_d] = (
#                 self.fill_value
#             )

#             # For segmentation tasks, you might want to modify the label as well
#             # Uncomment this if you want cutouts to affect the label too
#             """
#             label_copy[
#                 w1:w1 + cutout_w,
#                 h1:h1 + cutout_h,
...
#             # If SDF is present, apply cutout to it too
#             if self.with_sdf:
#                 sdf_copy[w1 : w1 + cutout_w, h1 : h1 + cutout_h, d1 : d1 + cutout_d] = (
#                     self.fill_value
#                 )

#         # Return the transformed sample
#         if self.with_sdf:
#             return {"image": image_copy, "label": label_copy, "sdf": sdf_copy}
#         else:
#             return {"image": image_copy, "label": label_copy}


# class WeakStrongAugmentPlus(object):
#     def __init__(
#         self, output_size, p_color=0.8, p_blur=0.2, p_cutout=0.5, flag_rot=True
#     ):
#         self.output_size = output_size
#         self.randrotflip = RandomRotFlip()
#         self.randcrop = RandomCrop(output_size)
#         # 初始化RandomCutout，设置适当的参数
#         self.randcutout = RandomCutout(
#             cutout_size=(output_size[0] // 8, output_size[1] // 8, output_size[2] // 8),
#             n_holes=3,
#             fill_value=0,
#         )
#         self.p_cutout = p_cutout  # 控制应用cutout的概率
#         self.randcolor = RandomBrightnessContrast(
#             brightness_limit=0.5, contrast_limit=0.5, prob=p_color
#         )
#         self.randblur = RandomGaussianNoise(sigma=[0.1, 1.0], apply_prob=p_blur)
...
#         # 创建用于cutout的样本字典
#         cutout_sample = {"image": image_strong, "label": label}

#         # 根据概率应用cutout增强到强增强版本
#         if np.random.random() < self.p_cutout:
#             cutout_result = self.randcutout(cutout_sample)
#             image_strong = cutout_result["image"]
