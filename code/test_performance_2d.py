...
parser.add_argument(
    "--flag_check_with_best_stu", default=False, action="store_true", help=""
)
parser.add_argument(
...
def Inference(FLAGS):
    ...
    snapshot_path = "{}/{}_{}_labeled/{}".format(
        FLAGS.res_path, FLAGS.exp, FLAGS.labeled_num, FLAGS.model
    )
    if FLAGS.flag_check_with_best_stu:
        test_save_path = "{}/{}_{}_labeled/{}_predictions_stu/".format(
            FLAGS.res_path, FLAGS.exp, FLAGS.labeled_num, FLAGS.model
        )
    else:
    ...
    if FLAGS.flag_check_with_best_stu:
        save_model_path = os.path.join(
            snapshot_path, "{}_best_stu_model.pth".format(FLAGS.model)
        )
    else:
        save_model_path = os.path.join(
            snapshot_path, "{}_best_tea_model.pth".format(FLAGS.model)
        )
