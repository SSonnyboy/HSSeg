...
class DiceLossWeightmap(nn.Module):
    def __init__(self, n_classes):
        super(DiceLossWeightmap, self).__init__()
        self.n_classes = n_classes
    ...
    def _dice_loss(self, score, target, weight_map):
        ...
        smooth = 1e-5
        intersect = (
            torch.sum(score * target)
            if weight_map is None
            else torch.sum(score * target * weight_map)
        )
        y_sum = (
            torch.sum(target * target)
            if weight_map is None
            else torch.sum(target * target * weight_map)
        )
        z_sum = (
            torch.sum(score * score)
            if weight_map is None
            else torch.sum(score * score * weight_map)
        )
        loss = (2 * intersect + smooth) / (z_sum + y_sum + smooth)
        loss = 1 - loss
        return loss
...
def cm_loss_acdc(
    ...
    patch_mask = 1 - mask  # 前景
    loss_dice = (
        dice(output_soft, img_l.unsqueeze(1), mask=mask.unsqueeze(1) * img_map)
        * image_weight
    )
    loss_dice += (
        dice(
            output_soft,
            patch_l.unsqueeze(1),
            mask=patch_mask.unsqueeze(1) * patch_map,
        )
        * patch_weight
    )
    loss_ce = (
        image_weight * (CE(output, img_l) * mask * img_map).sum() / (mask.sum() + 1e-16)
    )
    loss_ce += (
        patch_weight
        * (CE(output, patch_l) * patch_mask * patch_map).sum()
        / (patch_mask.sum() + 1e-16)
    )  # loss = loss_ce
    return (loss_dice + loss_ce) / 2
