...
from networks.resnet import resnet34_2d, resnet18_2d
...
class Res18UNet_2d(nn.Module):
    def __init__(self, in_chns, class_num):
        super(Res18UNet_2d, self).__init__()
...
class Res34UNet_2d(nn.Module):
    def __init__(self, in_chns, class_num):
        super(Res34UNet_2d, self).__init__()
    ...
    def forward(self, x, is_feature=False):
        ...
        output, features = self.decoder(feature)
        if is_feature:
            return output, feature[-2], feature[-1], features[0], features[1]
        return output
...
class UNet_3D(nn.Module):
    def __init__(self, in_channel=1, out_channel=2, training=False):
        ...
        self.encoder1 = nn.Conv3d(
            in_channel, 32, 3, stride=1, padding=1
        )  # b, 16, 10, 10
    ...
    def forward(self, x):
        ...
        out = F.relu(
            F.interpolate(self.decoder2(out), scale_factor=(2, 2, 2), mode="trilinear")
        )
        out = torch.add(out, t3)
        output2 = self.map2(out)
        out = F.relu(
            F.interpolate(self.decoder3(out), scale_factor=(2, 2, 2), mode="trilinear")
        )
        out = torch.add(out, t2)
        output3 = self.map3(out)
        out = F.relu(
            F.interpolate(self.decoder4(out), scale_factor=(2, 2, 2), mode="trilinear")
        )
        out = torch.add(out, t1)

        out = F.relu(
            F.interpolate(self.decoder5(out), scale_factor=(2, 2, 2), mode="trilinear")
        )
