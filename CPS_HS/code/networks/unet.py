...
class Decoder(nn.Module):
    ...
    def forward(self, feature):
        ...
        x_2 = self.up3(x_1, x1)
        x_3 = self.up4(x_2, x0)
        output = self.out_conv(x_3)
        return output, [x_0, x_1]
...
class Decoder_DS(nn.Module):
    def __init__(self, params):
        ...
        self.out_conv_dp4 = nn.Conv2d(
            self.ft_chns[4], self.n_class, kernel_size=3, padding=1
        )
        self.out_conv_dp3 = nn.Conv2d(
            self.ft_chns[3], self.n_class, kernel_size=3, padding=1
        )
        self.out_conv_dp2 = nn.Conv2d(
            self.ft_chns[2], self.n_class, kernel_size=3, padding=1
        )
        self.out_conv_dp1 = nn.Conv2d(
