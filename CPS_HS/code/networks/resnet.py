...
def conv3x3_2d_bn_relu(in_planes, out_planes, stride=1):
    return nn.Sequential(
        conv3x3_2d(in_planes, out_planes, stride), nn.BatchNorm2d(out_planes), nn.ReLU()
    )
...
class ResNet2d(nn.Module):
    ...
    def __init__(
        ...
        if deep_stem:
            self.conv1 = nn.Sequential(
                conv3x3_2d_bn_relu(in_channel, 32, stride=2),
                conv3x3_2d_bn_relu(32, 32, stride=1),
                conv3x3_2d(32, 64, stride=1),
            )
        else:
...
def resnet18_2d(**kwargs):
    return ResNet2d(BasicBlock2d, [2, 2, 2, 2], **kwargs)
...
def resnet34_2d(**kwargs):
    return ResNet2d(BasicBlock2d, [3, 4, 6, 3], **kwargs)
